...
export default function Profile() {
  ...
  const handleListDelete = async (listingId) => {
    try {
      const res = await fetch(import.meta.env.VITE_BACKEND_API + `/api/user/listing/delete${listingId}`, {
        method: 'DELETE',
      })
      const data = await res.json();
      if (data.success === false) {
  ...
  return (
    <div className='w-full px-4 mx-auto md:w-96'>
      ...
      <div className="flex justify-between mt-5">
        ...
        <span className='text-red-700 cursor-pointer float-right mx-auto' onClick={handleSignOut}>Sign out</span>
      </div>
      {error && (
        <p className="text-red-700 mt-2">{error}</p>
      )}
      {successful && (
        <p className="text-green-700 mx-auto mt-2 w-fit">updated successfully</p>
      )}
